...
import { RANK_CONFIG, auraToNextTier, tierProgress, RANK_ORDER } from '@/utils/ranks'
import { formatAura, cn } from '@/utils/helpers'
import type { Session, RankTier } from '@/types'
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (!profile) return
    profileService.getSessionHistory(profile.id, 30)
      .then(s => { setSessions(s as Session[]); setLoading(false) })
  }, [profile?.id])
  ...
  const totalMinutes  = sessions.reduce((acc, s) => acc + Math.floor(s.duration_completed / 60), 0)

  const catBreakdown = sessions.reduce<Record<string, number>>((acc, s) => {
  ...
  const handleSaveUsername = async () => {
    ...
    setSaveErr(null)
    try {
      const updated = await profileService.updateProfile({ username: newUsername.toLowerCase() })
      setProfile(updated)
      setEditing(false)
    } catch (err: any) {
  ...
  return (
    <div className="min-h-full">
      <PageHeader title="Profile" subtitle="Your arc, your legacy." />
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="card-purple p-6 relative overflow-hidden">
          ...
          <div className="relative flex items-start gap-5">
            ...
            <div className="relative flex-shrink-0">
              ...
              <div className={cn('absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-[#0f0f1a] flex items-center justify-center text-[8px]', rankCfg.bg)}>
                {rankCfg.icon}
              </div>
            </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                {editing ? (
                  ...
                  <>
                    ...
                    <button
                      onClick={() => { setEditing(true); setNewUsername(profile.username) }}
              ...
              {saveErr && <p className="text-red-400 text-xs font-hud mb-1">{saveErr}</p>}
            ...
            {/* Share button */}
            <ShareButton username={profile.username} className="flex-shrink-0" />
          </div>
        </div>

        {/* ── STATS ── */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard label="Weekly Aura"  value={<AuraCounter value={profile.weekly_aura} />} variant="purple" icon={Zap} />
          <StatCard label="Total Aura"   value={formatAura(profile.total_aura)} variant="blue" icon={Zap} />
          <StatCard label="Daily Streak" value={`🔥 ${profile.daily_streak}`} variant="gold" icon={Flame} />
          <StatCard label="Sessions"     value={totalSessions} sub={`${totalMinutes} min total`} icon={Target} />
        </div>

        {/* ── RANK PROGRESS ── */}
        <div className="card p-5">
          <div className="section-header mb-3">Rank Journey</div>
          <div className="flex gap-1 mb-3">
            {RANK_ORDER.map((tier, i) => {
              const cfg     = RANK_CONFIG[tier as RankTier]
              const reached = RANK_ORDER.indexOf(profile.rank_tier as RankTier) >= i
              return (
                <div
                  key={tier}
                  title={cfg.label}
                  ...
                  style={{
                    background: reached
                      ? tier === profile.rank_tier ? '#9333ea' : '#334155'
          ...
          <div className="flex items-center justify-between">
            <span className={cn('font-hud text-xs font-bold', rankCfg.color)}>
        ...
        <div className="grid lg:grid-cols-5 gap-4">
          ...
          <div className="lg:col-span-2 card p-5">
            <div className="section-header mb-3">Activity Breakdown</div>
            <div className="space-y-2.5">
                ...
                return (
                  <div key={cat}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="font-hud text-xs text-slate-600">{count} · {pct}%</span>
                    </div>
              ...
              {Object.keys(catBreakdown).length === 0 && (
                <p className="text-slate-700 font-hud text-xs">No sessions yet.</p>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-3 card divide-y divide-white/[0.04]">
            <div className="px-4 py-3 section-header mb-0">Recent Sessions</div>
            {sessions.slice(0, 12).map(s => {
              const task = (s as any).task
              const mins = Math.floor(s.duration_completed / 60)
              return (
                <div key={s.id} className="flex items-center gap-3 px-4 py-3">
                  <span className="text-lg flex-shrink-0">{ICONS[task?.category] ?? '🎯'}</span>
                  <div className="flex-1 min-w-0">
                    <div className="font-hud font-semibold text-xs text-slate-300 truncate">
                      {task?.title ?? 'Task'}
                    </div>
                    <div className="font-hud text-[10px] text-slate-600">
                      {new Date(s.started_at).toLocaleDateString()} · {mins}min
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    {s.completed
                      ? <span className="font-hud font-bold text-xs text-purple-400">+{s.aura_earned.toLocaleString()}</span>
                      : <span className="font-hud text-xs text-slate-700">—</span>
                    }
                    <div className="font-hud text-[9px] mt-0.5">
                      {s.completed
                        ? <span className="text-green-600">DONE</span>
                        : <span className="text-slate-700">QUIT</span>}
                    </div>
                  </div>
                </div>
              )
